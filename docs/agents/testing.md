...
- Keep focused module tests beside the implementation as `src/**/*.test.ts(x)` when they exercise a module-owned behavior through relative imports.
- Put app-boundary, cross-module, tooling, and shared integration tests under `tests/`, mirroring the relevant source ownership.
