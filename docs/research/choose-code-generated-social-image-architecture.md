...
Research for [Choose the code-generated social-image architecture](https://github.com/levinbaenninger/website/issues/15), captured on 2026-07-28.

## Decision

Use the installed Next.js **`next/og` `ImageResponse`** through colocated `opengraph-image.tsx` and `twitter-image.tsx` metadata files. Generate the Portfolio, Blog, and every visible Article image during `next build`; do not introduce a public query-driven image endpoint or request-time rendering.

`next/og` is already part of `next@16.3.0-preview.6`, integrates directly with the App Router metadata-file conventions, emits PNG, and is sufficient for a bounded 1200×630 social card. The installed implementation bundles `@vercel/og@0.11.1`, Satori, Yoga Wasm, and Resvg Wasm; selecting it adds no direct dependency or native deployment binary. [Installed `@vercel/og` package metadata](../../node_modules/next/dist/compiled/@vercel/og/package.json); [installed `ImageResponse` documentation](../../node_modules/next/dist/docs/01-app/03-api-reference/04-functions/image-response.md).

Generated metadata images are statically optimized and cached by default unless they use request-time APIs, dynamic configuration, or uncached data. The metadata files automatically provide the image URL, type, size, and alt tags to the applicable route segment. [Installed Open Graph and Twitter image documentation](../../node_modules/next/dist/docs/01-app/03-api-reference/03-file-conventions/01-metadata/opengraph-image.md); [official Next.js metadata-file documentation](https://nextjs.org/docs/app/api-reference/file-conventions/metadata/opengraph-image).

Use both metadata filenames so Next owns both `og:image*` and `twitter:image*` tags. They share one pure image composition and the same renderer-neutral input rather than maintaining two designs. For an Article, `generateImageMetadata` returns exactly one descriptor so its title-derived alt text can be generated from the same Blog projection as the pixels. In Next.js 16, both the image function's `params` and its `id` are promises. [Installed `generateImageMetadata` documentation](../../node_modules/next/dist/docs/01-app/03-api-reference/04-functions/generate-image-metadata.md); [official `generateImageMetadata` documentation](https://nextjs.org/docs/app/api-reference/functions/generate-image-metadata).
...
Portfolio data / Blog social-image projection
                       |
                       v
thin opengraph-image.tsx and twitter-image.tsx files
                       |
                       v
...
- The Article segment has one pair whose static parameters come from the same visible-slug projection as `/blog/[slug]`. Each Published Article is rendered during production build. Local development may additionally resolve Drafts.
- The image path does not call `headers()`, `cookies()`, Draft Mode, `connection()`, a clock, random-number or UUID APIs, remote fetches, or uncached data. It does not set `dynamic`, `revalidate`, or an Edge runtime.
...
This follows the established rule that production consumes static imports and Next-generated outputs rather than reading Article sources through `fs`. [Article compilation decision](https://github.com/levinbaenninger/website/issues/7). Next's metadata-file documentation states that these special Route Handlers are cached by default and are generated at build time when they avoid dynamic or uncached inputs. [Next.js metadata-file reference](https://nextjs.org/docs/app/api-reference/file-conventions/metadata).
...
The Article value is derived from the validated title, never separately authored. It is returned in the one-item `generateImageMetadata` result for both metadata files. The static Portfolio and Blog files export `alt` directly. There is no empty alt, filename alt, generic “social image” text, or renderer-authored fallback.
...
- `src/app` owns the six metadata-file adapters, their Next.js exports, static parameter wiring, and translation of missing Blog data to a 404.
...
| Static build | Inspect `vp run build` output and request built image URLs twice | Metadata image routes are prerendered/cached, not request-time renderers; bodies are stable and successful. |
...
This decision fixes the renderer, build-time lifecycle, metadata-file integration, OG/Twitter parity, output and alt contracts, deterministic input rules, title-overflow behavior, Draft/redirect behavior, ownership seams, and verification requirements.
