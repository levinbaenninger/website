...
import {
  ...
  sortBlogDiagnostics,
} from "@/features/blog/tooling/diagnostics.ts";
import {
  checkArticleManifest,
  generateArticleManifest,
  watchArticleSource,
} from "@/features/blog/tooling/source-manifest.ts";
import type { BlogToolPaths } from "@/features/blog/tooling/source-manifest.ts";
...
const createRepository = async (): Promise<BlogToolPaths> => {
  ...
  const articlesRoot = path.join(repositoryRoot, "src/features/blog/content");
